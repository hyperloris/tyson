export class Constants {
  public static readonly DEFAULT_SERIALIZE_NULLS = false;
  public static readonly JSON_PROPERTY_METADATA_KEY = "jsonProperty";
  public static readonly FUNCTION_TYPE_LOWERCASE = "function";
  public static readonly ARRAY_TYPE = "Array";
  public static readonly BOOLEAN_TYPE = "Boolean";
  public static readonly BOOLEAN_TYPE_LOWERCASE = "boolean";
  public static readonly NUMBER_TYPE = "Number";
  public static readonly NUMBER_TYPE_LOWERCASE = "number";
  public static readonly STRING_TYPE = "String";
  public static readonly STRING_TYPE_LOWERCASE = "string";
}
